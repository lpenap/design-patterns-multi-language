...
| Spec | Scope | PRs |
|---|---|---|
| 034 | Opener: the 23 catalogue entries with empty implementations (README shows them pending), Enterprise category, references 17–20, `docs/principles.md`, `docs/future/genai-patterns.md`, `docs/future/api-design-patterns.md`, README links | 1 |
...
| 17 | Phase 5 scope | Coverage review of 2026-09-25 against Gamma et al., Head First Design Patterns, Head First OOA&D, Fowler (PoEAA), Geewax (API Design Patterns) and Lakshmanan & Hapke (Generative AI Design Patterns): the two classic catalogues are complete. Added to the existing categories: Object Pool (creational), Null Object (behavioural, absorbing Fowler's Special Case), Monitor, Read/Write Lock, Future/Promise, Thread Pool and Guarded Suspension (concurrency). Confirmed 2026-09-25 |
...
| 20 | Principles | Head First OOA&D adds no pattern; it is cited for principles. `docs/principles.md` states the principles the patterns lean on (encapsulate what varies, program to an interface, favour composition over inheritance, open-closed, single responsibility, delegation) with citations to [17], [9] and [8]; each pattern doc names the principles it relies on. Confirmed 2026-09-25 |
| 21 | Phase 5 granularity | One PR per pattern per language, Java → Python → TypeScript → JavaScript, merged before the next; the Java PR carries the doc; all catalogue entries land in spec 034 with empty implementations; parity `strict` unless a spec records a reason for `loose`. Confirmed 2026-09-25 |
